...
Wave::Wave(std::string name, bool loop, float gain) :
    mName(name),
    mSource(0),
    mBuffer(0),
...
bool Wave::loadWave(std::string filePath)
{
...
void Wave::setPosition(vec3 newPos)
{
    mPosition = newPos;
    ALfloat temp[3] = {mPosition.x, mPosition.y, mPosition.z};
    alSourcefv(mSource, AL_POSITION, temp);
}
void Wave::setVelocity(vec3 newVel)
{
    mVelocity = newVel;
...
bool Wave::checkError(std::string name)
{
    switch (alGetError())
    {
    case AL_NO_ERROR:
